...
import { vec2, vec2Add, vec2Mul, radians } from './math.ts';
...
export class Turtle {
  position: Vector2;
  angle: number;
  stack: Array<{ position: Vector2; angle: number }>;
  path: Vector2[];
  penDown: boolean;
...
export const mandelbrot = (c: { real: number; imag: number }, maxIterations: number = 100): number => {
  let z = { real: 0, imag: 0 };
  let iterations = 0;
