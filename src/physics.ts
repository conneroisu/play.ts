...
import { vec2, vec2Add, vec2Mul, vec2Distance, vec2Sub, vec2Normalize, vec2Length } from './math.ts';
 ...
 */
export const friction = (velocity: Vector2, coefficient: number = 0.1): Vector2 => 
  vec2Mul(velocity, -coefficient);
 ...
 */
export const drag = (velocity: Vector2, coefficient: number = 0.01): Vector2 => {
...
export class Particle {
  ...
  applySpring(anchor: Vector2, restLength: number = 0, stiffness: number = 0.1): void {
...
export class VerletParticle {
  ...
  update(deltaTime: number = 1): void {
    ...
    this.position = vec2Add(
      vec2Add(this.position, velocity),
      accelStep
    );
