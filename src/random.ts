...
export const randomDistribution = (count: number, min: number, max: number): number[] => {
  const values: number[] = [];
...
export const randomWalk = (steps: number, stepSize: number = 1, dimensions: number = 2): number[][] => {
  const walk: number[][] = [];
...
export const sampleWeighted = <T>(array: T[], weights: number[], count: number): T[] => {
  const result: T[] = [];
  for (let i = 0; i < count; i++) {
