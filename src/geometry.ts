 ...
 */
export const rect = (x: number, y: number, width: number, height: number): Rectangle => ({
  x,
  y,
  width,
  height
 ...
 */
export const rectIntersection = (a: Rectangle, b: Rectangle): Rectangle | null => {
  const x = max(a.x, b.x);
  const y = max(a.y, b.y);
...
export const line = (start: Point, end: Point): Line => ({ start, end });
...
export const regularPolygon = (center: Point, radius: number, sides: number, rotation: number = 0): Polygon => {
...
export const quadraticBezier = (t: number, p0: Point, p1: Point, p2: Point): Point => {
  const u = 1 - t;
...
export const hexGrid = (center: Point, radius: number, rings: number): Point[] => {
...
export const triangle = (p1: Point, p2: Point, p3: Point): Polygon => [p1, p2, p3];
...
export const star = (center: Point, outerRadius: number, innerRadius: number, points: number): Polygon => {
