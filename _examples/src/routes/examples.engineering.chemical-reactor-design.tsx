...
export const Route = createFileRoute(
	...
	component: ChemicalReactorDesignExample,
});
...
function ChemicalReactorDesignExample() {
	...
	const calculateConversion = () => {
		...
		switch (reactor.reactorType) {
			case "cstr":
				...
				const denominator =
					1 +
					k * tau * Math.pow(reactor.concentration, kinetics.reactionOrder - 1);
				conversion =
					(k *
						tau *
						Math.pow(reactor.concentration, kinetics.reactionOrder - 1)) /
					denominator;
				break;
	...
	const calculatePerformance = () => {
		...
		const reactionRate =
			...
			Math.pow(
				reactor.concentration * (1 - conversion),
				kinetics.reactionOrder,
			);
							...
							<div>
								...
								<Slider
									...
									onValueChange={([value]) =>
										setKinetics({
											...kinetics,
											preExponentialFactor: Math.pow(10, value),
										})
									}
									min={3}
									max={12}
