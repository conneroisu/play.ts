...
describe("Performance Tests", () => {
  describe("Math Operations Performance", () => {
    test("basic math functions performance", () => {
      ...
      console.log(
        `Math functions (${iterations} ops): clamp=${clampTime}ms, lerp=${lerpTime}ms, map=${mapTime}ms`,
      );
    });
    ...
    test("trigonometric operations performance", () => {
      ...
      console.log(
        `Trigonometric ops (${iterations} ops): fromAngle=${fromAngleTime}ms, rotate=${rotateTime}ms`,
      );
    });
  });
  ...
  describe("Color Operations Performance", () => {
    test("color creation and conversion performance", () => {
      ...
      console.log(
        `Color ops (${iterations} ops): rgb=${rgbTime}ms, hsl=${hslTime}ms, conversion=${conversionTime}ms`,
      );
    });
  ...
  describe("Random Generation Performance", () => {
    ...
    test("seeded random performance", () => {
      ...
      console.log(
        `Seeded random (${iterations} ops): next=${seededTime}ms, gaussian=${gaussianTime}ms`,
      );
    });
  ...
  describe("Geometry Operations Performance", () => {
    test("point operations performance", () => {
      ...
      for (let i = 0; i < 1000; i++) {
        points.push(
          point(Math.random() * 200 - 100, Math.random() * 200 - 100),
        );
      }
      ...
      console.log(
        `Point operations (${iterations} ops): distance=${distanceTime}ms, lerp=${lerpTime}ms`,
      );
    });
  ...
  describe("Memory and Allocation Performance", () => {
    test("vector creation allocation performance", () => {
      ...
      console.log(
        `Vector creation (${iterations} allocations): ${createTime}ms`,
      );
    });
    ...
    test("color creation allocation performance", () => {
      ...
      console.log(
        `Color creation (${iterations} allocations): ${createTime}ms`,
      );
    });
  ...
  describe("Stress Tests", () => {
    test("massive vector calculation stress test", () => {
      ...
      for (let i = 0; i < numVectors; i++) {
        vectors.push(
          vec2(Math.random() * 1000 - 500, Math.random() * 1000 - 500),
        );
      }
      ...
      console.log(
        `Stress test (${numVectors} vectors, ${numIterations} iterations): ${stressTime}ms`,
      );
    });
    ...
    test("particle system simulation stress test", () => {
      ...
      console.log(
        `Particle simulation (${numParticles} particles, ${numFrames} frames): ${simulationTime}ms`,
      );
    });
    ...
    test("complex geometry operations stress test", () => {
      ...
      for (let i = 0; i < numPolygons; i++) {
        ...
        for (let j = 0; j < sides; j++) {
          ...
          vertices.push(
            point(50 + Math.cos(angle) * radius, 50 + Math.sin(angle) * radius),
          );
        }
      ...
      console.log(
        `Geometry stress test (${numPolygons} polygons, ${numTestPoints} points): ${geometryTime}ms`,
      );
    });
  });
  ...
  describe("Comparative Performance", () => {
    test("compare vector operations vs manual calculations", () => {
      ...
      console.log(
        `Vector operations comparison (${iterations} ops): vector=${vectorTime}ms, manual=${manualTime}ms`,
      );
    ...
    test("compare color operations vs direct RGB manipulation", () => {
      ...
      console.log(
        `Color operations comparison (${iterations} ops): color=${colorTime}ms, direct=${directTime}ms`,
      );
