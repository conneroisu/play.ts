...
import { createMockCanvas, createMockContext2D, getMockContext, expectNoError } from './utils/test-helpers.ts';
...
describe('Visual and Canvas Tests', () => {
  ...
  describe('Transformation Operations', () => {
    test('translate moves coordinate system', () => {
      ...
      expect(
        calls.some((call: any) => call.method === 'translate' && call.args[0] === 50 && call.args[1] === 100)
      ).toBe(true);
    });
    ...
    test('rotate rotates coordinate system', () => {
      ...
      expect(calls.some((call: any) => call.method === 'rotate' && Math.abs(call.args[0] - PI / 4) < 1e-10)).toBe(true);
    });
    ...
    test('scale changes coordinate system scale', () => {
      ...
      expect(calls.some((call: any) => call.method === 'scale' && call.args[0] === 2 && call.args[1] === 3)).toBe(true);
    });
    ...
    test('uniform scale with single parameter', () => {
      ...
      expect(calls.some((call: any) => call.method === 'scale' && call.args[0] === 1.5 && call.args[1] === 1.5)).toBe(
        true
      );
    });
    ...
    test('polygon star pattern drawing', () => {
      ...
      const createStar = (
        ...
        for (let i = 0; i < points * 2; i++) {
          ...
          vertices.push(point(centerX + Math.cos(angle) * radius, centerY + Math.sin(angle) * radius));
        }
  ...
  describe('Canvas State Management', () => {
    ...
    test('matrix stack maintains proper nesting', () => {
      ...
      const restoreCount = calls.filter(call => call.method === 'restore').length;
