...
export const expectCloseTo = (received: number, expected: number, precision: number = 5) => {
...
export const expectArrayCloseTo = (received: number[], expected: number[], precision: number = 5) => {
...
export const expectArrayInRange = (values: number[], min: number, max: number) => {
...
export const expectColorCloseTo = (
  received: { r: number; g: number; b: number },
...
export const measurePerformance = (fn: () => void, iterations: number = 1000): number => {
...
export const expectPerformance = (fn: () => void, maxTimeMs: number, iterations: number = 1000) => {
...
export const generateRandomNumbers = (count: number, min: number = -100, max: number = 100): number[] => {
...
export const generateRandomIntegers = (count: number, min: number = -100, max: number = 100): number[] => {
  const numbers: number[] = [];
  for (let i = 0; i < count; i++) {
...
export const generateRandomVectors2D = (count: number, min: number = -100, max: number = 100) => {
...
export const expectNormalDistribution = (
  ...
  expect(Math.abs(actualStdDev - expectedStdDev)).toBeLessThan(expectedStdDev * tolerance);
};
...
export const createMockContext2D = (): CanvasRenderingContext2D => {
  ...
  const context = {
    ...
    // Transform methods
    save: (...args: any[]) => calls.push({ method: 'save', args }),
    restore: (...args: any[]) => calls.push({ method: 'restore', args }),
    translate: (...args: any[]) => calls.push({ method: 'translate', args }),
    rotate: (...args: any[]) => calls.push({ method: 'rotate', args }),
    scale: (...args: any[]) => calls.push({ method: 'scale', args }),
...
export const generateTestData = {
  ...
  matrices3x3: () => [
    ...
    [2, 0, 0, 0, 2, 0, 0, 0, 1], // Scale 2x
    [1, 0, 5, 0, 1, 3, 0, 0, 1], // Translate (5, 3)
    [0, -1, 0, 1, 0, 0, 0, 0, 1] // Rotate 90°
  ]
};

// Fuzzing helpers
export const fuzzTest = (testFunction: (input: any) => void, inputGenerator: () => any, iterations: number = 100) => {
  for (let i = 0; i < iterations; i++) {
