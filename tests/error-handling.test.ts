...
import { expectError, expectNoError, createMockCanvas } from './utils/test-helpers.ts';
...
describe('Error Handling Tests', () => {
  ...
  describe('Color Error Handling', () => {
    ...
    test('color interpolation with invalid colors', () => {
      const invalidColor1 = { r: NaN, g: 0, b: 255 };
      const invalidColor2 = { r: 255, g: Infinity, b: 0 };
  ...
  describe('Geometry Error Handling', () => {
    test('point operations with invalid coordinates', () => {
      ...
      expectNoError(() => pointDistance(point(Infinity, 0), point(-Infinity, 0)));
    });
    ...
    test('polygon operations with invalid vertices', () => {
      ...
      expectError(() => polygon(point(0, 0), point(1, 1)), 'at least 3 vertices'); // Two vertices

      // Valid triangle with NaN coordinates
      const invalidTriangle = polygon(point(NaN, 0), point(1, Infinity), point(0, -Infinity));
    ...
    test('regular polygon with invalid parameters', () => {
      ...
      expectError(() => regularPolygon(point(0, 0), 10, -5), 'at least 3 sides'); // Negative sides
    ...
    test('circle from points with invalid input', () => {
      ...
      expectNoError(() => circleFromPoints(point(0, 0), point(1, 1), point(2, 2)));

      // Points with NaN/Infinity
      expectNoError(() => circleFromPoints(point(NaN, 0), point(1, Infinity), point(0, -Infinity)));
    });
  });
  ...
  describe('Play Class Error Handling', () => {
    ...
    test('drawing operations with invalid parameters', () => {
      ...
      expectNoError(() => playInstance.drawPolygon([point(NaN, 0), point(1, Infinity)])); // Invalid points
  ...
  describe('Performance Under Error Conditions', () => {
    test('error handling does not significantly impact performance', () => {
      ...
      console.log(`Performance with errors: normal=${normalDuration}ms, with errors=${errorDuration}ms`);
    });
