...
import { PI, TWO_PI, vec2Length } from "../src/math.ts";
import {
	FractalNoise,
	fractalNoise,
	noise,
...
describe("Random utilities", () => {
	...
	describe("Noise generators", () => {
		...
		test("SimpleNoise is deterministic", () => {
			...
			expect(noise1.noise3D(1.5, 2.5, 3.5)).toBe(
				noise2.noise3D(1.5, 2.5, 3.5),
			);
		});
		...
		test("PerlinNoise with same seed produces same results", () => {
			...
			expect(perlin1.noise3D(1.5, 2.5, 3.5)).toBe(
				perlin2.noise3D(1.5, 2.5, 3.5),
			);
		});
